...
#include <stdlib.h>
#include <string.h>
#include <ctype.h> // isspace() 사용을 위해 추가
...
struct token_unit {
    ...
    char operand[MAX_OPERAND][20]; // Operand
    char comment[100];  // Comment
    int locctr;            // Pass 1에서 계산된 주소
    int line_number;    // 원본 소스 라인 번호 (출력용)
...
struct symbol_unit {
    char name[20]; // 심볼 이름
    int addr;      // 심볼의 주소
    struct symbol_unit *next; // 해시 테이블 충돌 시 Chaining을 위함
};
typedef struct symbol_unit symbol;

// 전역 변수
token *token_table[MAX_LINES];
inst *inst_table[MAX_INST];     // OPTAB
symbol *symbol_table[MAX_INST]; // SYMTAB
...
int searchsymbol(char *cp) {
    ...
    strcpy(dp, cp);
    if (cp[strlen(cp) - 2] == ',') // X비트를 사용하는 피연산자 구분
        dp = strtok(dp, ",");
...
int main(int argc, char *argv[]) {
    if (argc < 2) {
        fprintf(stderr, "Usage: %s <input_filename.asm>\n", argv[0]);
        return 1;
    }
    ...
    // 3. 파일 읽기 및 파싱
    read_and_parse_file(argv[1]);

    // 4. Pass 1 수행
    if (!perform_pass_1()) {
        fprintf(stderr, "Errors occurred during Pass 1. Assembly aborted.\n");
        free_all_memory();
        return 1;
    }
